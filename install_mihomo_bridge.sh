...
show_logo() {
    ...
    echo -e "${BLUE}====================================================================${PLAIN}"
    echo -e "${BLUE}        Sing-box (Inbounds)    <=========>    Mihomo (Outbound)     ${PLAIN}"
    echo -e "${BLUE}                      多协议入站桥接出站部署脚本                    ${PLAIN}"
    echo -e "${BLUE}====================================================================${PLAIN}"
    echo ""
}

show_logo
log_info "开始安装 Sing-box 入站对接本地 Mihomo 出站一键部署脚本..."

create_sb_tool() {
cat > /usr/local/bin/sb <<'EOF'
#!/bin/bash
# Sing-box 极简快捷管理工具 (Mihomo 桥接版)
...
while true; do
    ...
    echo -e "${BLUE}        快捷管理工具 sb (Mihomo 桥接版)${PLAIN}"
    echo -e "${BLUE}==================================================${PLAIN}"
    echo "1. 查看已配置的节点分享链接"
...
# 交互获取本地已有的 Mihomo Socks5 端口
echo "=================================================="
echo "          配置本地 Mihomo 出站对接"
echo "=================================================="
read -p "请输入本地已在运行的 Mihomo Socks5 代理端口 [默认 7890]: " opt_m_port
[[ -n "$opt_m_port" ]] && MIHOMO_PORT=$opt_m_port
log_info "出站将通过 Socks5 转发到本地 127.0.0.1:${MIHOMO_PORT}"
...
log_info "正在生成 sing-box 配置文件 (包含对 Mihomo 的出站代理)..."
inbounds=()
enabled_tags=()
...
log_info "出站默认以 Socks5 对接到您本地运行在 127.0.0.1:${MIHOMO_PORT} 的 Mihomo 代理中。"
